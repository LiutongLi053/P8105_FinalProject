...
Although our final analysis focuses on general health, mental health represents a key component of overall health and provides an informative starting point for exploratory analysis. Therefore, we first examine the distributions of poor mental health and depression prevalence across census tracts. Both variables exhibit slightly right-skewed, unimodal distributions. The average prevalence of poor mental health is `r round(mental_summary$mean_mhlth, 3)`, ranging from `r mental_summary$range_mhlth`%. Depression shows a similar pattern, with an average prevalence of `r round(mental_summary$mean_dep, 3)` and a range of `r mental_summary$range_dep`%.
```{r , echo=FALSE}
plot_mhlth
plot_depression
```
...
This simple regression reveals a moderate positive association, supporting our initial expectation that states with higher energy insecurity tend to report higher prevalence of poor mental health. The relationship is statistically significant and practically meaningful. Notably, because poor mental health is a key component of overall general health, this pattern also suggests a broader connection: states facing greater energy insecurity may experience wider declines in overall health. The model explains about 44% of the variation in poor mental health across states, indicating that energy insecurity is important but not the sole contributor to population health outcomes.
